...
## Appended, never reordered: the name at each index is what a Run save
## round-trips through.
enum SystemSelectionSource { RUN_RANDOM = 0, QUICK_RANDOM, QUICK_CONSTRUCTED, HEADLESS_PINNED, BOSS_ATTACK }
...
const SYSTEM_SELECTION_SOURCE_NAMES := ["RUN_RANDOM", "QUICK_RANDOM", "QUICK_CONSTRUCTED", "HEADLESS_PINNED", "BOSS_ATTACK"]
